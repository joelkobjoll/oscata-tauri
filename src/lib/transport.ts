...
const WEB_ROUTES: Record<string, [string, string] | ((args: Record<string, unknown>) => [string, string, unknown?])> = {
  ...
  // Dynamic paths:
  queue_download:       (a) => ["POST", "/downloads", { ftp_path: (a.ftp_path ?? a.ftpPath), filename: a.filename, media_title: (a.media_title ?? a.mediaTitle ?? null) }],
  cancel_download:      (a) => ["DELETE", `/downloads/${a.id}`],
  ...
  apply_tmdb_match:     (a) => ["PUT", `/media/${a.id}/match`, { tmdb_id: a.tmdbId, media_type: a.mediaType }],
  search_tmdb:          (a) => ["POST", "/tmdb/search", { query: a.query, media_type: a.mediaType, year: a.year }],
};
...
export async function call<T>(command: string, args: Record<string, unknown> = {}): Promise<T> {
  ...
  if (!route) throw new Error(`[transport] No web route for command "${command}"`);
